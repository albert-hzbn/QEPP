...
static double extract_total_energy(const std::string& path) {
    ...
    while (std::getline(in, line)) {
        ...
        if (lo.find("!    total energy") != std::string::npos ||
            ...
            if (eq != std::string::npos) {
                ...
                if (ss >> val >> unit)
                    energy = (to_lower(unit) == "ry") ? val*Ry2eV : val;
            }
        }
    }
    if (std::isnan(energy))
...
// ─────────────────────────────────────────────────────────────────────────────
//  Print / write report (stdout + optional file)
// ─────────────────────────────────────────────────────────────────────────────
void print_elastic_report(const ElasticResults& res,
    ...
    sep();
    buf.str(""); buf<<std::fixed<<std::setprecision(3);
    buf<<"  Lambda (1st Lame, λ = K - 2G/3) = "<<std::setw(10)<<res.lambdaH<<" GPa\n"
       <<"  Mu    (shear,    μ = G)          = "<<std::setw(10)<<res.muH    <<" GPa\n";
    W(buf.str());
    ...
    buf<<"  Chen et al. 2011  Hv = 2(k²G)^0.585 - 3, k=G_H/K_H  :"
    ...
    buf<<"  Linear compressibility β = Σ_j S_ij  (10⁻³/GPa, dL/L per unit pressure):\n"
       <<std::setprecision(4)
       <<"    β_x = "<<std::setw(9)<<res.beta_x*1e3
       <<"    β_y = "<<std::setw(9)<<res.beta_y*1e3
       <<"    β_z = "<<std::setw(9)<<res.beta_z*1e3<<"  (×10⁻³ GPa⁻¹)\n";
    W(buf.str());
    ...
    buf<<"  Debye temp. Θ_D       = "<<std::setw(10)<<res.thetaDebye<<" K"
       <<"  [Anderson, J. Phys. Chem. Solids 24 (1963) 909]\n";
    buf<<std::setprecision(3)
       <<"  Gruneisen param. γ    = "<<std::setw(10)<<res.gruneisen
       <<"  [Belomestnykh & Tesleva, Tech. Phys. 49 (2004) 1304]\n";
    buf<<std::setprecision(4)
       <<"  κ_min (CWP)           = "<<std::setw(10)<<res.kMinClarke<<" W/(m·K)"
       <<"  [Cahill, Watson & Pohl, PRB 46 (1992) 6131]\n"
       <<"  κ_Slack (300K)        = "<<std::setw(10)<<res.kSlack   <<" W/(m·K)"
