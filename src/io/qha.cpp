...
void qha_generate_volumes(const std::string& qeInputPath,
    ...
    // Derive prefix for phonon DOS path
    ...
    fsum << "# Format: volume(Ang^3)   energy(Ry)   path/to/" << qePrefix << ".phonon.dos\n";
    fsum << "#\n";
    ...
    for (int i = 0; i < nVolumes; ++i) {
        ...
        try {
            generate_phonon_inputs(scfName, dfptDir, dfptOpts, /*verbose=*/false);
        ...
        // Summary line: volume  energy(TODO)  path/to/<prefix>.phonon.dos
        // matdyn.x writes <prefix>.phonon.dos to its CWD (the volume subdir, not dfpt/).
        fsum << std::fixed << std::setprecision(6)
             << std::setw(20) << Vi
             << std::setw(20) << "TODO_fill_energy"
             << make_vol_dir_name(i, nVolumes) << "/" << qePrefix << ".phonon.dos\n";
...
QhaVolumePoint compute_qha_volume_from_dos(const std::string& dosPath,
    ...
    // 2. Build temperature grid (use caller-supplied grid or default 0..1500 K step 10 K)
...
std::vector<QhaVolumePoint> read_qha_summary(const std::string& summaryPath,
    ...
    for (const auto& raw : lines) {
        ...
        QhaVolumePoint pt = compute_qha_volume_from_dos(dpath.string(), vol, ery, temps);
